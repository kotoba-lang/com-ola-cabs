(ns ola_cabs.main
  "Kotodama WASM entrypoint for the Ola_cabs clean-room actor (L4) — Clojure port.
  ...
  facts (`ola_cabs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ola_cabs")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ola_cabs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ola_cabs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ola_cabs-compat/ready)
